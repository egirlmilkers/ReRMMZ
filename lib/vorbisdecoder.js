...
var DYNAMIC_BASE = 72928, DYNAMICTOP_PTR = 7232;
var INITIAL_INITIAL_MEMORY = Module["INITIAL_MEMORY"] || 1048576;
...
var wasmBinaryFile = "vorbisdecoder.wasm";
...
var _stb_vorbis_close = Module["_stb_vorbis_close"] = function () {
	return (_stb_vorbis_close = Module["_stb_vorbis_close"] = Module["asm"]["e"]).apply(null, arguments)
};
var _free = Module["_free"] = function () {
	return (_free = Module["_free"] = Module["asm"]["f"]).apply(null, arguments)
};
var _stb_vorbis_decode_frame_pushdata = Module["_stb_vorbis_decode_frame_pushdata"] = function () {
	return (_stb_vorbis_decode_frame_pushdata = Module["_stb_vorbis_decode_frame_pushdata"] = Module["asm"]["g"]).apply(null, arguments)
};
var _stb_vorbis_open_pushdata = Module["_stb_vorbis_open_pushdata"] = function () {
	return (_stb_vorbis_open_pushdata = Module["_stb_vorbis_open_pushdata"] = Module["asm"]["h"]).apply(null, arguments)
};
...
class VorbisDecoder {
	constructor(context, decodeHandler, errorHandler) {
		VorbisDecoder.id = (VorbisDecoder.id || 0) % 1e3;
		this.id = VorbisDecoder.id++;
		this.context = context;
		this.decodeHandler = decodeHandler;
		this.errorHandler = errorHandler;
		VorbisDecoder.instances = VorbisDecoder.instances || [];
		VorbisDecoder.instances[this.id] = this
	}

	destroy() {
		const message = {id: this.id, destroy: true};
		VorbisDecoder.worker.postMessage(message);
		delete VorbisDecoder.instances[this.id]
	}

	send(arrayBuffer, isLoaded) {
		const message = {id: this.id, arrayBuffer: arrayBuffer, isLoaded: isLoaded};
		VorbisDecoder.worker.postMessage(message)
	}

	onMessage(message) {
		const channels = message.data.length;
		if (channels > 0) {
			const length = message.data[0].length;
			const buffer = this.context.createBuffer(channels, length, message.sampleRate);
			for (let i = 0; i < channels; i++) {
				buffer.getChannelData(i).set(message.data[i])
			}
...
if (typeof WorkerGlobalScope === "function") {
	class VorbisWorkerDecoder {
		...
		onMessage(message) {
			if (message.arrayBuffer) {
				if (this.isOpen()) {
					this.replace(message.arrayBuffer)
				} else {
					this.open(message.arrayBuffer)
				}
			}
			if (message.isLoaded) {
				this.isLoaded = message.isLoaded
			}
			this.decode()
		}
		...
		open(arrayBuffer) {
			...
			this.channels = 2;
			this.bufferMax = 8192;
			this.bufferPtr = Module._malloc(this.bufferMax);
			this.usedPtr = Module._malloc(4);
			this.outputPtr = Module._malloc(4);
			this.samplesPtr = Module._malloc(4);
			this.makeBuffer();
			this.v = Module._stb_vorbis_open_pushdata(this.bufferPtr, this.bufferSize, this.usedPtr, null, null);
			this.byteOffset += Module.getValue(this.usedPtr, "i32");
			this.channels = Module.getValue(this.v + 4, "i32");
		...
		close() {
			if (this.isOpen()) {
				Module._stb_vorbis_close(this.v);
				Module._free(this.bufferPtr);
				Module._free(this.usedPtr);
				Module._free(this.outputPtr);
				Module._free(this.samplesPtr);
				this.v = null
			}
		...
		makeBuffer() {
			const totalBytes = this.arrayBuffer.byteLength;
			const remainingBytes = totalBytes - this.byteOffset;
			this.bufferSize = Math.min(remainingBytes, this.bufferMax);
			const array = new Uint8Array(this.arrayBuffer, this.byteOffset, this.bufferSize);
			Module.HEAPU8.set(array, this.bufferPtr)
		}

		decodeFrame() {
			this.makeBuffer();
			const used = Module._stb_vorbis_decode_frame_pushdata(this.v, this.bufferPtr, this.bufferSize, null, this.outputPtr, this.samplesPtr);
			if (used > 0) {
				const output = Module.getValue(this.outputPtr, "i32");
				const samples = Module.getValue(this.samplesPtr, "i32");
				this.byteOffset += used;
				for (let i = 0; i < this.channels; i++) {
					const output_i = Module.getValue(output + i * 4, "i32");
					const array = new Float32Array(Module.HEAPU8.buffer, output_i, samples);
					this.arrays[i].push(this.resample(array))
				}
		...
		decode() {
			if (this.isOpen()) {
				for (; ;) {
					...
					if (this.totalSamples >= this.sampleRate * 5) {
						this.sendBack();
						setTimeout(this.decode.bind(this));
						break
					}
				}
			}
		...
		resample(array) {
			if (this.sampleScale === 1) {
				return array.slice()
			} else {
				const scale = this.sampleScale;
				const expanded = new Float32Array(array.length * scale);
				for (let i = 0; i < array.length; i++) {
					for (let j = 0; j < scale; j++) {
						expanded[i * scale + j] = array[i]
					}
				}
				return expanded
			}
		...
		concatenate() {
			for (let i = 0; i < this.channels; i++) {
				this.data[i] = new Float32Array(this.totalSamples * this.sampleScale);
				let offset = 0;
				for (const array of this.arrays[i]) {
					this.data[i].set(array, offset);
					offset += array.length
				}
			}
		}
		...
		sendBack() {
			if (this.totalSamples > 0) {
				this.concatenate();
				const message = {id: this.id, sampleRate: this.sampleRate * this.sampleScale, data: this.data};
				const transfer = [];
				for (let i = 0; i < this.channels; i++) {
					transfer.push(this.data[i].buffer)
				}
	...
	VorbisWorkerDecoder.instances = [];
	self.addEventListener("message", e => {
		const id = e.data.id;
		const instances = VorbisWorkerDecoder.instances;
		if (!instances[id]) {
			instances[id] = new VorbisWorkerDecoder(id)
		}
		if (instances[id]) {
			instances[id].onMessage(e.data)
		}
		if (e.data.destroy) {
			instances[id].close();
			delete instances[id]
