...
export class Tilemap extends PIXI.Container {
	...
	static isVisibleTile(tileId) {
		return tileId > 0 && tileId < Tilemap.TILE_ID_MAX;
	}

	static isAutotile(tileId) {
		return tileId >= Tilemap.TILE_ID_A1;
	}

	static getAutotileKind(tileId) {
		return Math.floor((tileId - Tilemap.TILE_ID_A1) / 48);
	}

	static getAutotileShape(tileId) {
		return (tileId - Tilemap.TILE_ID_A1) % 48;
	}

	static makeAutotileId(kind, shape) {
		return Tilemap.TILE_ID_A1 + kind * 48 + shape;
	}

	static isSameKindTile(tileID1, tileID2) {
		if (Tilemap.isAutotile(tileID1) && Tilemap.isAutotile(tileID2)) {
			return (
				Tilemap.getAutotileKind(tileID1) === Tilemap.getAutotileKind(tileID2)
			);
		} else {
	...
	static isTileA1(tileId) {
		return tileId >= Tilemap.TILE_ID_A1 && tileId < Tilemap.TILE_ID_A2;
	}

	static isTileA2(tileId) {
		return tileId >= Tilemap.TILE_ID_A2 && tileId < Tilemap.TILE_ID_A3;
	}

	static isTileA3(tileId) {
		return tileId >= Tilemap.TILE_ID_A3 && tileId < Tilemap.TILE_ID_A4;
	}

	static isTileA4(tileId) {
		return tileId >= Tilemap.TILE_ID_A4 && tileId < Tilemap.TILE_ID_MAX;
	}

	static isTileA5(tileId) {
		return tileId >= Tilemap.TILE_ID_A5 && tileId < Tilemap.TILE_ID_A1;
	}

	static isWaterTile(tileId) {
		if (Tilemap.isTileA1(tileId)) {
			return !(
				tileId >= Tilemap.TILE_ID_A1 + 96 && tileId < Tilemap.TILE_ID_A1 + 192
			);
		} else {
			return false;
		}
	...
	static isWaterfallTile(tileId) {
		if (tileId >= Tilemap.TILE_ID_A1 + 192 && tileId < Tilemap.TILE_ID_A2) {
	...
	static isWallSideTile(tileId) {
		return (
			(Tilemap.isTileA3(tileId) || Tilemap.isTileA4(tileId)) &&
	...
	static isFloorTypeAutotile(tileId) {
		return (
			(Tilemap.isTileA1(tileId) && !Tilemap.isWaterfallTile(tileId)) ||
			Tilemap.isTileA2(tileId) ||
...
Tilemap.CombinedLayer = class extends PIXI.Container {
	constructor() {
		super();
		for (let i = 0; i < 2; i++) {
