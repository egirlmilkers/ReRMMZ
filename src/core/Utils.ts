...
export class Utils {
	...
	static checkRMVersion(version) {
		const array1 = Utils.RPGMAKER_VERSION.split(".");
	...
	static isOptionValid(name) {
		...
		if (Utils.isNwjs() && nw.App.argv.length > 0) {
	...
	static setEncryptionInfo(hasImages, hasAudio, key) {
		...
		Utils._hasEncryptedImages = hasImages;
		Utils._hasEncryptedAudio = hasAudio;
		Utils._encryptionKey = key;
	}
	...
	static hasEncryptedImages() {
		return Utils._hasEncryptedImages;
	}
	...
	static hasEncryptedAudio() {
		return Utils._hasEncryptedAudio;
	}
	...
	static decryptArrayBuffer(source) {
		...
		const key = Utils._encryptionKey.match(/.{2}/g);
		for (let i = 0; i < 16; i++) {
