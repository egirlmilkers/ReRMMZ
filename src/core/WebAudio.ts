...
export class WebAudio {
	...
	static initialize() {
		...
		WebAudio._createMasterGainNode();
	...
	static setMasterVolume(value) {
		WebAudio._masterVolume = value;
		WebAudio._resetVolume();
	}

	static _createContext() {
		try {
			const AudioContext = window.AudioContext || window.webkitAudioContext;
			WebAudio._context = new AudioContext();
		} catch (e) {
			WebAudio._context = null;
		}
	}
	...
	static _currentTime() {
		return WebAudio._context ? WebAudio._context.currentTime : 0;
	}

	static _createMasterGainNode() {
		const context = WebAudio._context;
		if (context) {
			WebAudio._masterGainNode = context.createGain();
			WebAudio._resetVolume();
			WebAudio._masterGainNode.connect(context.destination);
		}
	}

	static _setupEventHandlers() {
		const onUserGesture = WebAudio._onUserGesture.bind(WebAudio);
		const onVisibilityChange = WebAudio._onVisibilityChange.bind(WebAudio);
	...
	static _resetVolume() {
		if (WebAudio._masterGainNode) {
			const gain = WebAudio._masterGainNode.gain;
			const volume = WebAudio._masterVolume;
