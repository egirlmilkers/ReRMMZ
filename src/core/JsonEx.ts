...
export class JsonEx {
	...
	static stringify(object) {
		return JSON.stringify(JsonEx._encode(object, 0));
	}
	 ...
	 */
	static parse(json) {
		return JsonEx._decode(JSON.parse(json));
	}
	...
	static _encode(value, depth) {
		...
		if (type === "[object Object]" || type === "[object Array]") {
			...
			for (const key of Object.keys(value)) {
				value[key] = JsonEx._encode(value[key], depth + 1);
			}
		}
		return value;
	}
	...
	static _decode(value) {
		...
		if (type === "[object Object]" || type === "[object Array]") {
			...
			for (const key of Object.keys(value)) {
				value[key] = JsonEx._decode(value[key]);
			}
		}
		return value;
	}
