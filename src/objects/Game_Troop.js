...
export class Game_Troop extends Game_Unit {
	...
	troop() {
		return DataManager.$dataTroops[this._troopId];
	}
	...
	setup(troopId) {
		...
		for (const member of this.troop().members) {
			if (DataManager.$dataEnemies[member.enemyId]) {
	...
	letterTable() {
		return DataManager.$gameSystem.isCJK()
	...
	meetsConditions(page) {
		...
		if (c.enemyValid) {
			const enemy = DataManager.$gameTroop.members()[c.enemyIndex];
		...
		if (c.switchValid) {
			if (!DataManager.$gameSwitches.value(c.switchId)) {
				return false;
			}
		}
		return true;
	...
	goldTotal() {
		...
		return (
			members.reduce((r, enemy) => r + enemy.gold(), 0) * this.goldRate()
		);
	}

	goldRate() {
		return DataManager.$gameParty.hasGoldDouble() ? 2 : 1;
	}
	...
	isTpbTurnEnd() {
		...
		const turnMax = Math.max(
			...members.map((member) => member.turnCount()),
		);
