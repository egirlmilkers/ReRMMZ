...
Game_Interpreter.prototype.isOnCurrentMap = function () {
	return this._mapId === DataManager.$gameMap.mapId();
};

Game_Interpreter.prototype.setupReservedCommonEvent = function () {
	if (DataManager.$gameTemp.isCommonEventReserved()) {
		const commonEvent = DataManager.$gameTemp.retrieveCommonEvent();
		if (commonEvent) {
...
Game_Interpreter.prototype.updateWaitMode = function () {
	...
	switch (this._waitMode) {
		case "message":
			waiting = DataManager.$gameMessage.isBusy();
			break;
		case "transfer":
			waiting = DataManager.$gamePlayer.isTransferring();
			break;
		case "scroll":
			waiting = DataManager.$gameMap.isScrolling();
			break;
		case "route":
		...
		case "balloon":
			...
			break;
		case "gather":
			waiting = DataManager.$gamePlayer.areFollowersGathering();
			break;
		case "action":
...
Game_Interpreter.prototype.iterateActorId = function (param, callback) {
	if (param === 0) {
		DataManager.$gameParty.members().forEach(callback);
	} else {
		const actor = DataManager.$gameActors.actor(param);
		if (actor) {
			callback(actor);
		}
	}
...
Game_Interpreter.prototype.iterateActorEx = function (param1, param2, callback) {
	if (param1 === 0) {
		...
		this.iterateActorId(DataManager.$gameVariables.value(param2), callback);
	}
};
...
Game_Interpreter.prototype.iterateActorIndex = function (param, callback) {
	if (param < 0) {
		...
		const actor = DataManager.$gameParty.members()[param];
		if (actor) {
			callback(actor);
		}
	}
...
Game_Interpreter.prototype.iterateEnemyIndex = function (param, callback) {
	if (param < 0) {
		DataManager.$gameTroop.members().forEach(callback);
	} else {
		const enemy = DataManager.$gameTroop.members()[param];
		if (enemy) {
			callback(enemy);
		}
	}
...
Game_Interpreter.prototype.character = function (param) {
	if (DataManager.$gameParty.inBattle()) {
		...
		return DataManager.$gamePlayer;
	} else if (this.isOnCurrentMap()) {
		return DataManager.$gameMap.event(param > 0 ? param : this._eventId);
	} else {
		return null;
	}
};
...
Game_Interpreter.prototype.operateValue = function (
	...
	const value = operandType === 0 ? operand : DataManager.$gameVariables.value(operand);
...
Game_Interpreter.prototype.command101 = function (params) {
	...
	DataManager.$gameMessage.setFaceImage(params[0], params[1]);
	DataManager.$gameMessage.setBackground(params[2]);
	DataManager.$gameMessage.setPositionType(params[3]);
	DataManager.$gameMessage.setSpeakerName(params[4]);
	while (this.nextEventCode() === 401) {
		// Text data
		this._index++;
		DataManager.$gameMessage.add(this.currentCommand().parameters[0]);
	}
...
Game_Interpreter.prototype.setupChoices = function (params) {
	...
	DataManager.$gameMessage.setChoices(choices, defaultType, cancelType);
	DataManager.$gameMessage.setChoiceBackground(background);
	DataManager.$gameMessage.setChoicePositionType(positionType);
	DataManager.$gameMessage.setChoiceCallback((n) => {
...
Game_Interpreter.prototype.setupNumInput = function (params) {
	DataManager.$gameMessage.setNumberInput(params[0], params[1]);
};
...
Game_Interpreter.prototype.setupItemChoice = function (params) {
	DataManager.$gameMessage.setItemChoice(params[0], params[1] || 2);
};
...
Game_Interpreter.prototype.command105 = function (params) {
	...
	DataManager.$gameMessage.setScroll(params[0], params[1]);
...
Game_Interpreter.prototype.command111 = function (params) {
	...
	switch (params[0]) {
		case 0: // Switch
			result = DataManager.$gameSwitches.value(params[1]) === (params[2] === 0);
			break;
		case 1: // Variable
			value1 = DataManager.$gameVariables.value(params[1]);
			if (params[2] === 0) {
				value2 = params[3];
			} else {
				value2 = DataManager.$gameVariables.value(params[3]);
			}
			switch (params[4]) {
				case 0: // Equal to
		...
		case 2: // Self Switch
			if (this._eventId > 0) {
				...
				result = DataManager.$gameSelfSwitches.value(key) === (params[2] === 0);
			}
			break;
		case 3: // Timer
			if (DataManager.$gameTimer.isWorking()) {
				const sec = DataManager.$gameTimer.frames() / 60;
				if (params[2] === 0) {
					result = sec >= params[1];
				} else {
					result = sec <= params[1];
			...
			break;
		case 4: // Actor
			actor = DataManager.$gameActors.actor(params[1]);
			if (actor) {
				const n = params[3];
				switch (params[2]) {
					case 0: // In the Party
						result = DataManager.$gameParty.members().includes(actor);
						break;
					case 1: // Name
						result = actor.name() === n;
						break;
					case 2: // Class
						result = actor.isClass(DataManager.$dataClasses[n]);
						break;
					case 3: // Skill
						result = actor.hasSkill(n);
						break;
					case 4: // Weapon
						result = actor.hasWeapon(DataManager.$dataWeapons[n]);
						break;
					case 5: // Armor
						result = actor.hasArmor(DataManager.$dataArmors[n]);
						break;
					case 6: // State
			...
			break;
		case 5: // Enemy
			enemy = DataManager.$gameTroop.members()[params[1]];
			if (enemy) {
				switch (params[2]) {
					case 0: // Appeared
		...
		case 7: // Gold
			switch (params[2]) {
				case 0: // Greater than or equal to
					result = DataManager.$gameParty.gold() >= params[1];
					break;
			...
			break;
		case 8: // Item
			result = DataManager.$gameParty.hasItem(DataManager.$dataItems[params[1]]);
			break;
		case 9: // Weapon
			result = DataManager.$gameParty.hasItem(DataManager.$dataWeapons[params[1]], params[2]);
			break;
		case 10: // Armor
			result = DataManager.$gameParty.hasItem(DataManager.$dataArmors[params[1]], params[2]);
			break;
		case 11: // Button
			switch (params[2] || 0) {
				case 0:
		...
		case 12: // Script
			...
			break;
		case 13: // Vehicle
			result = DataManager.$gamePlayer.vehicle() === DataManager.$gameMap.vehicle(params[1]);
			break;
	}
...
Game_Interpreter.prototype.command117 = function (params) {
	const commonEvent = DataManager.$dataCommonEvents[params[0]];
	if (commonEvent) {
...
Game_Interpreter.prototype.command121 = function (params) {
	for (let i = params[0]; i <= params[1]; i++) {
		DataManager.$gameSwitches.setValue(i, params[2] === 0);
	}
	return true;
};
...
Game_Interpreter.prototype.command122 = function (params) {
	...
	switch (operand) {
		case 0: // Constant
			...
			break;
		case 1: // Variable
			value = DataManager.$gameVariables.value(params[4]);
			break;
		case 2: // Random
			value = params[4];
...
Game_Interpreter.prototype.gameDataOperand = function (type, param1, param2) {
	...
	switch (type) {
		case 0: // Item
			return DataManager.$gameParty.numItems(DataManager.$dataItems[param1]);
		case 1: // Weapon
			return DataManager.$gameParty.numItems(DataManager.$dataWeapons[param1]);
		case 2: // Armor
			return DataManager.$gameParty.numItems(DataManager.$dataArmors[param1]);
		case 3: // Actor
			actor = DataManager.$gameActors.actor(param1);
			if (actor) {
				switch (param2) {
					case 0: // Level
						return actor.level;
			...
			break;
		case 4: // Enemy
			enemy = DataManager.$gameTroop.members()[param1];
			if (enemy) {
				switch (param2) {
					case 0: // HP
						return enemy.hp;
		...
		case 5: // Character
			...
			break;
		case 6: // Party
			actor = DataManager.$gameParty.members()[param1];
			return actor ? actor.actorId() : 0;
		case 8: // Last
			return DataManager.$gameTemp.lastActionData(param1);
		case 7: // Other
			switch (param1) {
				case 0: // Map ID
					return DataManager.$gameMap.mapId();
				case 1: // Party Members
					return DataManager.$gameParty.size();
				case 2: // Gold
					return DataManager.$gameParty.gold();
				case 3: // Steps
					return DataManager.$gameParty.steps();
				case 4: // Play Time
					return DataManager.$gameSystem.playtime();
				case 5: // Timer
					return DataManager.$gameTimer.seconds();
				case 6: // Save Count
					return DataManager.$gameSystem.saveCount();
				case 7: // Battle Count
					return DataManager.$gameSystem.battleCount();
				case 8: // Win Count
					return DataManager.$gameSystem.winCount();
				case 9: // Escape Count
					return DataManager.$gameSystem.escapeCount();
			}
			break;
	}
	return 0;
...
Game_Interpreter.prototype.operateVariable = function (variableId, operationType, value) {
	try {
		const oldValue = DataManager.$gameVariables.value(variableId);
		switch (operationType) {
			case 0: // Set
				DataManager.$gameVariables.setValue(variableId, value);
				break;
			case 1: // Add
				DataManager.$gameVariables.setValue(variableId, oldValue + value);
				break;
			case 2: // Sub
				DataManager.$gameVariables.setValue(variableId, oldValue - value);
				break;
		...
		DataManager.$gameVariables.setValue(variableId, 0);
	}
};
...
Game_Interpreter.prototype.command123 = function (params) {
	if (this._eventId > 0) {
		...
		DataManager.$gameSelfSwitches.setValue(key, params[1] === 0);
	}
	return true;
};
...
Game_Interpreter.prototype.command124 = function (params) {
	if (params[0] === 0) {
		// Start
		DataManager.$gameTimer.start(params[1] * 60);
	} else {
		// Stop
		DataManager.$gameTimer.stop();
	}
	return true;
};
...
Game_Interpreter.prototype.command125 = function (params) {
	...
	DataManager.$gameParty.gainGold(value);
	return true;
};
...
Game_Interpreter.prototype.command126 = function (params) {
	...
	DataManager.$gameParty.gainItem(DataManager.$dataItems[params[0]], value);
	return true;
};
...
Game_Interpreter.prototype.command127 = function (params) {
	const value = this.operateValue(params[1], params[2], params[3]);
	DataManager.$gameParty.gainItem(DataManager.$dataWeapons[params[0]], value, params[4]);
	return true;
};
...
Game_Interpreter.prototype.command128 = function (params) {
	const value = this.operateValue(params[1], params[2], params[3]);
	DataManager.$gameParty.gainItem(DataManager.$dataArmors[params[0]], value, params[4]);
	return true;
};

// Change Party Member
Game_Interpreter.prototype.command129 = function (params) {
	const actor = DataManager.$gameActors.actor(params[0]);
	if (actor) {
		if (params[1] === 0) {
			// Add
			if (params[2]) {
				// Initialize
				DataManager.$gameActors.actor(params[0]).setup(params[0]);
			}
			DataManager.$gameParty.addActor(params[0]);
		} else {
			// Remove
			DataManager.$gameParty.removeActor(params[0]);
		}
	}
	return true;
};
...
Game_Interpreter.prototype.command132 = function (params) {
	DataManager.$gameSystem.setBattleBgm(params[0]);
	return true;
};

// Change Victory ME
Game_Interpreter.prototype.command133 = function (params) {
	DataManager.$gameSystem.setVictoryMe(params[0]);
	return true;
};
...
Game_Interpreter.prototype.command134 = function (params) {
	if (params[0] === 0) {
		DataManager.$gameSystem.disableSave();
	} else {
		DataManager.$gameSystem.enableSave();
	}
	return true;
};
...
Game_Interpreter.prototype.command135 = function (params) {
	if (params[0] === 0) {
		DataManager.$gameSystem.disableMenu();
	} else {
		DataManager.$gameSystem.enableMenu();
	}
	return true;
};
...
Game_Interpreter.prototype.command136 = function (params) {
	if (params[0] === 0) {
		DataManager.$gameSystem.disableEncounter();
	} else {
		DataManager.$gameSystem.enableEncounter();
	}
	DataManager.$gamePlayer.makeEncounterCount();
	return true;
};
...
Game_Interpreter.prototype.command137 = function (params) {
	if (params[0] === 0) {
		DataManager.$gameSystem.disableFormation();
	} else {
		DataManager.$gameSystem.enableFormation();
	}
	return true;
};
...
Game_Interpreter.prototype.command138 = function (params) {
	DataManager.$gameSystem.setWindowTone(params[0]);
	return true;
};

// Change Defeat ME
Game_Interpreter.prototype.command139 = function (params) {
	DataManager.$gameSystem.setDefeatMe(params[0]);
	return true;
};

// Change Vehicle BGM
Game_Interpreter.prototype.command140 = function (params) {
	const vehicle = DataManager.$gameMap.vehicle(params[0]);
	if (vehicle) {
...
Game_Interpreter.prototype.command201 = function (params) {
	if (DataManager.$gameParty.inBattle() || DataManager.$gameMessage.isBusy()) {
		return false;
	}
	let mapId, x, y;
	if (params[0] === 0) {
		...
		mapId = DataManager.$gameVariables.value(params[1]);
		x = DataManager.$gameVariables.value(params[2]);
		y = DataManager.$gameVariables.value(params[3]);
	}
	DataManager.$gamePlayer.reserveTransfer(mapId, x, y, params[4], params[5]);
...
Game_Interpreter.prototype.command202 = function (params) {
	...
	if (params[1] === 0) {
		...
		// Designation with variables
		mapId = DataManager.$gameVariables.value(params[2]);
		x = DataManager.$gameVariables.value(params[3]);
		y = DataManager.$gameVariables.value(params[4]);
	}
	const vehicle = DataManager.$gameMap.vehicle(params[0]);
	if (vehicle) {
...
Game_Interpreter.prototype.command203 = function (params) {
	...
	if (character) {
		if (params[1] === 0) {
			...
			// Designation with variables
			const x = DataManager.$gameVariables.value(params[2]);
			const y = DataManager.$gameVariables.value(params[3]);
...
Game_Interpreter.prototype.command204 = function (params) {
	if (!DataManager.$gameParty.inBattle()) {
		if (DataManager.$gameMap.isScrolling()) {
			this.setWaitMode("scroll");
			return false;
		}
		DataManager.$gameMap.startScroll(params[0], params[1], params[2]);
		if (params[3]) {
			this.setWaitMode("scroll");
		}
	}
...
Game_Interpreter.prototype.command205 = function (params) {
	DataManager.$gameMap.refreshIfNeeded();
...
Game_Interpreter.prototype.command206 = function () {
	DataManager.$gamePlayer.getOnOffVehicle();
	return true;
};

// Change Transparency
Game_Interpreter.prototype.command211 = function (params) {
	DataManager.$gamePlayer.setTransparent(params[0] === 0);
	return true;
};
...
Game_Interpreter.prototype.command212 = function (params) {
	...
	const character = this.character(this._characterId);
	if (character) {
		DataManager.$gameTemp.requestAnimation([character], params[1]);
		if (params[2]) {
...
Game_Interpreter.prototype.command213 = function (params) {
	...
	const character = this.character(this._characterId);
	if (character) {
		DataManager.$gameTemp.requestBalloon(character, params[1]);
		if (params[2]) {
...
Game_Interpreter.prototype.command214 = function () {
	if (this.isOnCurrentMap() && this._eventId > 0) {
		DataManager.$gameMap.eraseEvent(this._eventId);
	}
	return true;
};
...
Game_Interpreter.prototype.command216 = function (params) {
	if (params[0] === 0) {
		DataManager.$gamePlayer.showFollowers();
	} else {
		DataManager.$gamePlayer.hideFollowers();
	}
...
Game_Interpreter.prototype.command217 = function () {
	if (!DataManager.$gameParty.inBattle()) {
		DataManager.$gamePlayer.gatherFollowers();
...
Game_Interpreter.prototype.command221 = function () {
	...
	DataManager.$gameScreen.startFadeOut(this.fadeSpeed());
...
Game_Interpreter.prototype.command222 = function () {
	...
	DataManager.$gameScreen.startFadeIn(this.fadeSpeed());
	this.wait(this.fadeSpeed());
	return true;
};
...
Game_Interpreter.prototype.command223 = function (params) {
	DataManager.$gameScreen.startTint(params[0], params[1]);
	if (params[2]) {
		this.wait(params[1]);
	}
	return true;
...
Game_Interpreter.prototype.command224 = function (params) {
	DataManager.$gameScreen.startFlash(params[0], params[1]);
	if (params[2]) {
		this.wait(params[1]);
	}
	return true;
...
Game_Interpreter.prototype.command225 = function (params) {
	DataManager.$gameScreen.startShake(params[0], params[1], params[2]);
	if (params[3]) {
		this.wait(params[2]);
	}
	return true;
...
Game_Interpreter.prototype.command231 = function (params) {
	...
	// prettier-ignore
	DataManager.$gameScreen.showPicture(
...
Game_Interpreter.prototype.command232 = function (params) {
	const point = this.picturePoint(params);
	// prettier-ignore
	DataManager.$gameScreen.movePicture(
...
Game_Interpreter.prototype.picturePoint = function (params) {
	...
	if (params[3] === 0) {
		...
		// Designation with variables
		point.x = DataManager.$gameVariables.value(params[4]);
		point.y = DataManager.$gameVariables.value(params[5]);
	}
	return point;
};
...
Game_Interpreter.prototype.command233 = function (params) {
	DataManager.$gameScreen.rotatePicture(params[0], params[1]);
	return true;
};

// Tint Picture
Game_Interpreter.prototype.command234 = function (params) {
	DataManager.$gameScreen.tintPicture(params[0], params[1], params[2]);
	if (params[3]) {
		this.wait(params[2]);
	}
	return true;
...
Game_Interpreter.prototype.command235 = function (params) {
	DataManager.$gameScreen.erasePicture(params[0]);
	return true;
};
...
Game_Interpreter.prototype.command236 = function (params) {
	if (!DataManager.$gameParty.inBattle()) {
		DataManager.$gameScreen.changeWeather(params[0], params[1], params[2]);
		if (params[3]) {
			this.wait(params[2]);
		}
	}
...
Game_Interpreter.prototype.command243 = function () {
	DataManager.$gameSystem.saveBgm();
	return true;
};

// Resume BGM
Game_Interpreter.prototype.command244 = function () {
	DataManager.$gameSystem.replayBgm();
	return true;
};
...
Game_Interpreter.prototype.command281 = function (params) {
	if (params[0] === 0) {
		DataManager.$gameMap.enableNameDisplay();
	} else {
		DataManager.$gameMap.disableNameDisplay();
	}
	return true;
};
...
Game_Interpreter.prototype.command282 = function (params) {
	const tileset = DataManager.$dataTilesets[params[0]];
	const allReady = tileset.tilesetNames.map((tilesetName) => ImageManager.loadTileset(tilesetName)).every((bitmap) => bitmap.isReady());
	if (allReady) {
		DataManager.$gameMap.changeTileset(params[0]);
		return true;
	} else {
		return false;
	}
...
Game_Interpreter.prototype.command283 = function (params) {
	DataManager.$gameMap.changeBattleback(params[0], params[1]);
	return true;
};
...
Game_Interpreter.prototype.command284 = function (params) {
	// prettier-ignore
	DataManager.$gameMap.changeParallax(
...
Game_Interpreter.prototype.command285 = function (params) {
	...
	switch (params[1]) {
		case 0: // Terrain Tag
			value = DataManager.$gameMap.terrainTag(x, y);
			break;
		case 1: // Event ID
			value = DataManager.$gameMap.eventIdXy(x, y);
			break;
		case 2: // Tile ID (Layer 1)
		case 3: // Tile ID (Layer 2)
		case 4: // Tile ID (Layer 3)
		case 5: // Tile ID (Layer 4)
			value = DataManager.$gameMap.tileId(x, y, params[1] - 2);
			break;
		default:
			// Region ID
			value = DataManager.$gameMap.regionId(x, y);
			break;
	}
	DataManager.$gameVariables.setValue(params[0], value);
	return true;
};
...
Game_Interpreter.prototype.command301 = function (params) {
	if (!DataManager.$gameParty.inBattle()) {
		...
		if (params[0] === 0) {
			...
			troopId = DataManager.$gameVariables.value(params[1]);
		} else {
			// Same as Random Encounters
			troopId = DataManager.$gamePlayer.makeEncounterTroopId();
		}
...
Game_Interpreter.prototype.command303 = function (params) {
	if (!DataManager.$gameParty.inBattle()) {
		if (DataManager.$dataActors[params[0]]) {
...
Game_Interpreter.prototype.command321 = function (params) {
	const actor = DataManager.$gameActors.actor(params[0]);
	if (actor && DataManager.$dataClasses[params[1]]) {
...
Game_Interpreter.prototype.command335 = function (params) {
	this.iterateEnemyIndex(params[0], (enemy) => {
		enemy.appear();
		DataManager.$gameTroop.makeUniqueNames();
	});
	return true;
};
...
Game_Interpreter.prototype.command337 = function (params) {
	...
	DataManager.$gameTemp.requestAnimation(targets, params[1]);
	return true;
};
