...
export class Game_Actor extends Game_Battler {
    ...
    setup(actorId) {
        const actor = DataManager.$dataActors[actorId];
    ...
    actor() {
        return DataManager.$dataActors[this._actorId];
    }
    ...
    resetStateCounts(stateId) {
        ...
        this._stateSteps[stateId] = DataManager.$dataStates[stateId].stepsToRemove;
    }
    ...
    equipSlots() {
        const slots = [];
        for (let i = 1; i < DataManager.$dataSystem.equipTypes.length; i++) {
            slots.push(i);
        }
    ...
    tradeItemWithParty(newItem, oldItem) {
        if (newItem && !DataManager.$gameParty.hasItem(newItem)) {
            return false;
        } else {
    ...
    changeEquipById(etypeId, itemId) {
        ...
        if (this.equipSlots()[slotId] === 1) {
            this.changeEquip(slotId, DataManager.$dataWeapons[itemId]);
        } else {
            this.changeEquip(slotId, DataManager.$dataArmors[itemId]);
        }
    }
    ...
    bestEquipItem(slotId) {
        ...
        const items = DataManager.$gameParty.equipItems().filter((item) => item.etypeId === etypeId && this.canEquip(item));
        let bestItem = null;
    ...
    isBattleMember() {
        return DataManager.$gameParty.battleMembers().includes(this);
    }
    ...
    currentClass() {
        return DataManager.$dataClasses[this._classId];
    }
    ...
    skills() {
        ...
        for (const id of this._skills.concat(this.addedSkills())) {
            if (!list.includes(DataManager.$dataSkills[id])) {
                list.push(DataManager.$dataSkills[id]);
            }
        }
        return list;
    }
    ...
    displayLevelUp(newSkills) {
        ...
        for (const skill of newSkills) {
            DataManager.$gameMessage.add(TextManager.obtainSkill.format(skill.name));
        }
    }
    ...
    hasSkill(skillId) {
        return this.skills().includes(DataManager.$dataSkills[skillId]);
    }
    ...
    isSpriteVisible() {
        return DataManager.$gameSystem.isSideView();
    }
    ...
    performAttack() {
        ...
        const attackMotion = DataManager.$dataSystem.attackMotions[wtypeId];
        if (attackMotion) {
    ...
    performDamage() {
        ...
        if (this.isSpriteVisible()) {
            ...
            DataManager.$gameScreen.startShake(5, 5, 10);
        }
    ...
    onPlayerWalk() {
        ...
        if (DataManager.$gamePlayer.isNormal()) {
            this.turnEndOnMap();
    ...
    showAddedStates() {
        for (const state of this.result().addedStateObjects()) {
            if (state.message1) {
                DataManager.$gameMessage.add(state.message1.format(this._name));
            }
        }
    }
    ...
    showRemovedStates() {
        for (const state of this.result().removedStateObjects()) {
            if (state.message4) {
                DataManager.$gameMessage.add(state.message4.format(this._name));
            }
        }
    }
    ...
    turnEndOnMap() {
        if (DataManager.$gameParty.steps() % this.stepsForTurn() === 0) {
            this.onTurnEnd();
    ...
    checkFloorEffect() {
        if (DataManager.$gamePlayer.isOnDamageFloor()) {
    ...
    maxFloorDamage() {
        return DataManager.$dataSystem.optFloorDeath ? this.hp : Math.max(this.hp - 1, 0);
    }

    performMapDamage() {
        if (!DataManager.$gameParty.inBattle()) {
            DataManager.$gameScreen.startFlashForDamage();
        }
    }
