...
export class Game_Actor extends Game_Battler {
	...
	resetStateCounts(stateId) {
		...
		this._stateSteps[stateId] =
			DataManager.$dataStates[stateId].stepsToRemove;
	}
	...
	weapons() {
		return this.equips().filter(
			(item) => item && DataManager.isWeapon(item),
		);
	}

	armors() {
		return this.equips().filter(
			(item) => item && DataManager.isArmor(item),
		);
	}
	...
	displayLevelUp(newSkills) {
		...
		for (const skill of newSkills) {
			DataManager.$gameMessage.add(
				TextManager.obtainSkill.format(skill.name),
			);
		}
	}
