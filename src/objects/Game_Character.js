...
export class Game_Character extends Game_CharacterBase {
	...
	processMoveCommand(command) {
		...
		switch (command.code) {
			...
			case gc.ROUTE_SWITCH_ON:
				DataManager.$gameSwitches.setValue(params[0], true);
				break;
			case gc.ROUTE_SWITCH_OFF:
				DataManager.$gameSwitches.setValue(params[0], false);
				break;
	...
	deltaXFrom(x) {
		return DataManager.$gameMap.deltaX(this.x, x);
	}

	deltaYFrom(y) {
		return DataManager.$gameMap.deltaY(this.y, y);
	}
	...
	findDirectionTo(goalX, goalY) {
		...
		const mapWidth = DataManager.$gameMap.width();
		const nodeList = [];
		const openList = [];
		const closedList = [];
		const start = {};
		...
		start.y = this.y;
		start.g = 0;
		start.f = DataManager.$gameMap.distance(start.x, start.y, goalX, goalY);
		nodeList.push(start);
		...
		while (nodeList.length > 0) {
			...
			for (let j = 0; j < 4; j++) {
				...
				const x2 = DataManager.$gameMap.roundXWithDirection(
					x1,
					direction,
				);
				const y2 = DataManager.$gameMap.roundYWithDirection(
					y1,
					direction,
				);
				...
				if (index2 < 0 || g2 < nodeList[index2].g) {
					...
					neighbor.f =
						g2 +
						DataManager.$gameMap.distance(x2, y2, goalX, goalY);
		...
		const deltaX1 = DataManager.$gameMap.deltaX(node.x, start.x);
		const deltaY1 = DataManager.$gameMap.deltaY(node.y, start.y);
		if (deltaY1 > 0) {
			return 2;
		} else if (deltaX1 < 0) {
			return 4;
