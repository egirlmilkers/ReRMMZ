...
export class Game_Player extends Game_Character {
    ...
    performTransfer() {
        if (this.isTransferring()) {
            ...
            if (this._newMapId !== DataManager.$gameMap.mapId() || this._needsMapReload) {
    ...
    center(x, y) {
        return DataManager.$gameMap.setDisplayPos(x - this.centerX(), y - this.centerY());
    }
    ...
    canMove() {
        if (DataManager.$gameMap.isEventRunning() || DataManager.$gameMessage.isBusy()) {
            return false;
        }
    ...
    updateDashing() {
        ...
        if (this.canMove() && !this.isInVehicle() && !DataManager.$gameMap.isDashDisabled()) {
            this._dashing = this.isDashButtonPressed() || DataManager.$gameTemp.isDestinationValid();
        } else {
            this._dashing = false;
        }
    }
    ...
    triggerTouchActionD2(x2, y2) {
        if (DataManager.$gameMap.boat().pos(x2, y2) || DataManager.$gameMap.ship().pos(x2, y2)) {
    ...
    canEncounter() {
        return !DataManager.$gameParty.hasEncounterNone() && DataManager.$gameSystem.isEncounterEnabled() && !this.isInAirship() && !this.isMoveRouteForcing() && !this.isDebugThrough();
    }
    ...
    checkEventTriggerThere(triggers) {
        if (this.canStartLocalEvents()) {
            ...
            if (!DataManager.$gameMap.isAnyEventStarting() && DataManager.$gameMap.isCounter(x2, y2)) {
                const x3 = DataManager.$gameMap.roundXWithDirection(x2, direction);
                const y3 = DataManager.$gameMap.roundYWithDirection(y2, direction);
    ...
    isOnDamageFloor() {
        return DataManager.$gameMap.isDamageFloor(this.x, this.y) && !this.isInAirship();
    }
