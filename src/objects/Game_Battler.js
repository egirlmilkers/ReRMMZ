...
import { BattleManager, DataManager, SoundManager } from "managers";
import { Game_Action, Game_ActionResult, Game_BattlerBase } from "objects";
...
Game_Battler.prototype.tpbAcceleration = function () {
	const speed = this.tpbRelativeSpeed();
	const referenceTime = DataManager.$gameParty.tpbReferenceTime();
	return speed / referenceTime;
};

Game_Battler.prototype.tpbRelativeSpeed = function () {
	return this.tpbSpeed() / DataManager.$gameParty.tpbBaseSpeed();
};
...
Game_Battler.prototype.isStateAddable = function (stateId) {
	return this.isAlive() && DataManager.$dataStates[stateId] && !this.isStateResist(stateId) && !this.isStateRestrict(stateId);
};

Game_Battler.prototype.isStateRestrict = function (stateId) {
	return DataManager.$dataStates[stateId].removeByRestriction && this.isRestricted();
};
...
Game_Battler.prototype.maxSlipDamage = function () {
	return DataManager.$dataSystem.optSlipDeath ? this.hp : Math.max(this.hp - 1, 0);
};
