...
export class Window_Message extends Window_Base {
	...
	canStart() {
		return (
			DataManager.$gameMessage.hasText() &&
			!DataManager.$gameMessage.scrollMode()
		);
	}
	...
	updatePlacement() {
		...
		if (goldWindow) {
			goldWindow.y =
				this.y > 0 ? 0 : Graphics.boxHeight - goldWindow.height;
		}
	}
	...
	loadMessageFace() {
		this._faceBitmap = ImageManager.loadFace(
			DataManager.$gameMessage.faceName(),
		);
	}
