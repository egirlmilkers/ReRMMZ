...
export class Window_Base extends Window {
    ...
    fittingHeight(numLines) {
        return numLines * this.itemHeight() + DataManager.$gameSystem.windowPadding() * 2;
    }
    ...
    convertEscapeCharacters(text) {
        ...
        while (text.match(/\x1bV\[(\d+)\]/gi)) {
            text = text.replace(/\x1bV\[(\d+)\]/gi, (_, p1) => DataManager.$gameVariables.value(parseInt(p1)));
        }
    ...
    calcTextHeight(textState) {
        const lineSpacing = this.lineHeight() - DataManager.$gameSystem.mainFontSize();
